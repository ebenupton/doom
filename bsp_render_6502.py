...
def _mem_banked(mem):
    ...
    vxcache-valid clear into the wrong place (the walk then saw stale
...
def poke_init_frame_state(mem):
    ...
    records-pointer ground state + the 60-byte vxcache valid clear."""
    # VXCACHE_VALID + VDONE ride separate VRCACHE plane tails since
    # 2026-08-13 (57 B each) — wipe both by symbol
    _bk = 1 if _mem_banked(mem) else 0
    for name in ('VXCACHE_VALID_BASE', 'VDONE'):
