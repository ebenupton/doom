...
| 2026-04-19 |       5928 B  | **146297**         |       | **186073**         |       | **332370**|       | **mel: DCL-style per-span bbox skip.** Ports DCL's outer-reject pattern (which rejects spans whose bbox proves the line can never enter the aperture) to mel's pre-mark_solid wall edge emission pass. At ms_emit_lines entry, compute static seg Y bbox `(seg_top_min/max, seg_bot_min/max)` from the (unbiased) lo bytes of yt1/yt2/yb1/yb2. Per overlapping span, check whether ANY edge can emit using the precomputed POOL_OT/POOL_OB bbox: top emit possible iff `seg_top_max > POOL_OT AND seg_top_min < POOL_OB`; bot emit possible iff symmetrical. When neither edge can emit (span fully inside, fully above, or fully below the wall — all four "no-overlap" combinations), the entire 8-call interp + emission decision tree is skipped and we jump directly to `mel_next`. Sentinels disable the bbox check when (a) any yt/yb hi byte is non-zero (s16 seg outside u8 range) or (b) [sx1,sx2] doesn't cover [ilo,ihi] (overlap may extrapolate past the seg anchors, so endpoint bbox isn't a valid bound). Per-span check costs ~16 cyc; per-skip saves ~600-1100 cyc by eliding all four span_top + span_bot + seg_top + seg_bot interp calls plus the emission decision tree. S2 fires for 25/32 spans (78% — corridor view dominated by tall walls covering whole spans), saving ~1149 cyc/skip. S1 fires for 6/27 spans (22%); the small bbox setup overhead (~50 cyc/call × 19 = ~950 cyc) eats most of the gain there. Cycle change: S1 +510 (+0.35%), S2 −27987 (−13.1%), Grand −27477 (−7.6%). ROM +125 B. 0 new mismatches in 576-frame wide-test (12 pre-existing baseline mismatches preserved). 128/128 line-emission match across 8 positions × 16 angles. |
| 2026-04-19 |       6170 B  | **167482**         | +21185| **224935**         | +38862| **392417**| +60047| **tighten: emit secondary step edges (fb/ft).** Addresses missing step bottom/top lines (e.g. opening-scene line 20 at y=98) caused by commit 6b79fda's removal of DCL double-draw. When need_bt + ch>vz or need_bb + fh<vz, Python's draw_clipped emits BOTH the step edge (bt/bb — primary, at tighten's yt/yb) AND the front ceiling/floor edge (ft/fb — secondary). The 6502 previously emitted only the primary via ncf's nt/nb emission. Extended tighten API with `emit_sec_top/emit_sec_bot` flags (bits 2/3 of zp_tg_emit) + `yt_sec1/2` and `yb_sec1/2` (u8 at $B2-$B5, reusing DCL CB slots). Wrapper re-interpolates secondary values at post-remap anchors and clamps to u8. ncf_no_splits adds 4 new JSR sites (one per emission path pair) to `emit_sec_top_edge`/`emit_sec_bot_edge` helpers. Each helper has a constant-line fast path (yt_sec1==yt_sec2 → skip interp, reuse value as both endpoints) and a sloped path (2 interp_store calls at ox0/ox1 using seg's sx1/sx2 anchors). Tail-calls RASTER_ENTRY per emission. S1 pixel coverage improves 56% → 83% vs Python reference (1048 → 1540 non-black pixels of 1865); S2 improves 51% → 55% (fewer step cases + more crossovers where tg_overlap_sub's sec path isn't yet wired). Cost: S1 +21k (sec fires 6×), S2 +39k (sec fires 21×), Grand +60k. ROM +242 B. 0 new mismatches in wide-test. Remaining gaps: vertical lines (pre-existing, DCL always skipped), tg_overlap_sub crossover-case sec edges. |
| 2026-04-20 |       6259 B  | **141492**         | −25990| **222493**         | −2442 | **363985**| −28432| **DCL-all architecture: unified line emission via DCL + OR-mode rasteriser.** Reversed the previous two rounds and fixed the root-cause gaps from commit 6b79fda. Changes: (1) **Added vertical support to DCL** — `draw_clipped_line` now detects xl==xr via trampolined BNE/JMP to `dcl_vertical`, which walks the span list to find the first span containing column xl, interpolates top/bot at xl (constant-line fast path + full interp via `interp_store`), clips [ylo,yhi] to [top,bot], and emits one vertical segment. (2) **Disabled all mel/tighten emission** — `zp_ms_emit=0` skips `ms_emit_lines`; ncf_no_splits and tg_overlap_sub emission blocks elided. (3) **Switched rasteriser to `linedraw_or_reloc.bin`** — OR-mode prevents XOR double-draw erasure where verticals meet horizontals at aperture corners. (4) **Wrapper forwards ALL draw_clipped lines to DCL** — horizontal, sloped, and vertical. (5) **Matched Python's portal-continuation end-of-walk semantics** — `dcl_flush`'s final seg_start emission was unconditional; Python's draw_clipped loops exits without emitting when the last iteration was a portal-continue. Removed the flush emit so DCL matches. (6) **dcl_exit_check runs portal check when xend==xr** (was xend<xr only) so lines ending exactly at a portal boundary take the continuation path instead of emitting, matching Python. Pixel match across 6 scenes: 98.5% (9177/9319); S1 99.6%, S2 98.4%. Remaining 3.1% diff is dominated by off-by-one pixel placement at half-integer slope positions — pygame.draw.line and the 6502's XOR/OR Bresenham rasteriser pick different pixels (rasteriser-level issue, not clipping). 12 pre-existing wide-test mismatches preserved. Cost vs first-round baseline: S1 −4.3k (−2.9%), S2 +8.4k (+3.9%), Grand +4.1k (+1.2%). ROM +456 B cumulative today. |
| 2026-05-01 |       —       | **124092**         | −17400| **255072**         | −23300| **379164**| −31328| **Aperture-clip skip (Python-side, packed_render_seg).** Pre-check each line emission and skip lines that are entirely outside the visible spans — they would clip to nothing in the bank-2 span clipper anyway, so we save the dispatch + outer-reject cost. Three new EndpointClipSpans methods: `line_above_spans` and `line_below_spans` walk all overlapping spans and return True iff the line endpoints sit strictly above span_top (resp. below span_bot) at every overlap; `vertical_outside_spans` does the per-column check at a single sx. `Instrumented6502Spans` overrides each to bias by Y_BIAS before delegation. Skip points wired in: (a) whole need_bt block above-clipped via line_above_spans(bt) — drops bt horizontal + 2 step verts + optional front-ceil; (b) whole need_bb block below-clipped via line_below_spans(bb); (c) solid top horizontal above-clipped; (d) solid bot horizontal below-clipped; (e) solid V1/V2 full verticals out of span at sx via vertical_outside_spans; (f) solid APV1/APV2 aperture-edge verticals; (g) portal-plain elif top/bot single-line emissions; (h) portal APEDGE V1/V2 verticals; (i) need_bt/bb step verticals per-column; (j) need_bt front-ceil and need_bb front-floor secondary lines. All gated by `_AP_SKIP_ENABLE` (toggle off to reproduce unoptimised behaviour). Pixel coverage preserved across 9 test positions (verified by toggling and diffing line sets). Skip rates per emission type across 10 scenes: bt-block 46%, bb-block 26%, solid_top 39%, solid_bot 17%. Cycle change: S1 124092 (−17400, −13.2%), S2 255072 (−23300, −8.4%), Grand 379164 (−31328, −7.6%). Python-only — no ASM changes. The savings are entirely the outer-reject + dispatch overhead the bank-2 span clipper would have spent on the skipped lines (Instrumented6502Spans.draw_clipped forwards fewer lines to _span_clip_6502). |
| 2026-05-01 |       —       | **123730**         | −362  | **253410**         | −1662 | **377140**| −2024 | **Tighten no-op skip.** When a portal-with-steps seg's [yt, yb] strictly contains every overlapping span's [span_top, span_bot] (tested by line_above_spans(yt) AND line_below_spans(yb)), tighten cannot narrow any span — its per-span dominance check would discover this but the span walk + interp work isn't free.  Skip the entire `Instrumented6502Spans.tighten` call (both the Python state mutation and the 6502 shadow), yielding identical span state.  Fires 17 times across 10 test scenes (~2/scene), most often in T NE (5) and T N (4).  Cumulative cycle savings (vs no-opt baseline): S1 −8390 (−6.4%), S2 −24962 (−9.0%), Grand across S1+S2 −33352 (−7.5%).  10-scene grand total 1304138 → 1214404 = −89734 (−6.9%).  Pixel coverage preserved; pre-existing skip points unchanged. |
| 2026-05-01 |       —       | **123730**         |     0 | **253410**         |     0 | **377140**|     0 | **Drop dead top_dom/bot_dom line_survives calls.** Cleanup, follow-up to the tighten no-op skip.  The `top_dom`/`bot_dom` flags computed via `clips.line_survives(bt)` and `clips.line_survives(bb)` flow into `Instrumented6502Spans.tighten` → `super().tighten` (ignored) and into the deferred queue tuple → `packed_render_subsector` flush → tighten (also ignored). `_span_clip_6502.tighten` doesn't accept them, and `span_clip.asm` tighten doesn't reference them.  The only consumer was the now-superseded full-6502 front-end's queue_tighten which has its own ASM line_survives (independent of this Python pipeline).  Replaced both calls with `top_dom = bot_dom = False` stubs in fp_render_seg, packed_render_seg, and clip_and_draw_6502 (preserves tuple/positional layout for `*op[1:]` unpack at flush).  ccyc unchanged (these calls didn't go through the bank-2 simulator); savings are Python-side CPU only — 6 line_survives calls × ~30 segs × per-frame eliminated.  Pixel coverage preserved. |
| 2026-05-01 |       —       |        — |     — |        — |     — |       — |     — | **Combined draw-and-tighten attempt — REVERTED.**  Restored tighten primary emission in span_clip.asm ncf_no_splits and tg_overlap_sub paths (re-added the IF EMIT_LINES blocks deleted by the 2026-04-20 DCL-all commit), and dropped bt/bb horizontals + portal-plain elif lines from packed_render_seg's draw_clipped lines list — saves one span walk per portal-with-steps seg.  Got cumulative ccyc savings vs (1)+(2) baseline of ~6%: S1 124k→117k (−5.9%), S2 253k→241k (−5.1%).  Pixel coverage regressed: 130–340 lost pixels per scene out of ~1500–2300, dominated by off-by-one positions on sloped lines.  **Root cause: rasteriser-level Bresenham mismatch.**  DCL emits each line as one continuous segment with portal continuation collapsing across span boundaries; tighten emits per-overlap-range, breaking lines at every span seam.  The OR rasteriser draws each fragment separately, accumulating Bresenham error per-fragment rather than over the whole line — gives a different sloped pixel pattern.  Fixing this would require porting DCL's portal-continuation seam-tracking logic into tighten (substantial 6502 work, recreating most of DCL's complexity) or making the rasteriser slope-deterministic per line direction (rasteriser-level change).  Reverted — the off-by-one regressions across hundreds of pixels per scene aren't acceptable for a 6% win.  Lesson: in a unified DCL-all architecture, line emission must go through a single path with consistent slope tracking; tighten can't easily "re-emit" the same edge because per-span fragments don't bresenham-equivalent to the original line.  Reverted to (1)+(2) state: 6.9% ccyc savings preserved. |
| 2026-05-01 |       —       |        — |     — |        — |     — |       — |     — | **Combined draw-and-tighten — Single-span variant ALSO REVERTED.**  Second attempt at (3): only enable tighten emission when the seg covers exactly one span at flush time (so the per-span fragment IS the whole line — no Bresenham mismatch with DCL's portal-continuation emission).  Implementation: re-enabled tighten emission code in ncf_no_splits and tg_overlap_sub (gated on emit_top/emit_bot bits in zp_tg_emit), added Python-side `covers_one_span` check, also rejected when pending solid/tighten ops in the deferred queue overlap [sx1, sx2] (could split spans before flush).  Caller dropped bt/bb from draw_clipped lines list ONLY when single-span confirmed.  ccyc savings would have been ~5%.  Pixel coverage **partially regressed**: S1_E was clean (0 diffs across 10 single-span fires!), but SE/doorway/T_N had 60-78 off-by-one pixels per scene.  Diagnosed but not fixed: DCL's portal-continuation collapses lines across multiple spans into a single rasteriser call, while tighten emission goes per-physical-span even when my Python check thinks coverage is single-span.  The single-span check fires based on Python's spans state at emission time, but at flush time the 6502 ASM tighten walks its OWN spans list, and the rasteriser sees per-span emissions even when conceptually one line.  Reverted.  The fundamental architectural issue is that DCL uses a stateful walk (seg_start tracked across spans, single rasteriser call at line end) while tighten emits one rasteriser call per per-span overlap — these are not pixel-equivalent in general because the Bresenham slope error accumulates differently.  To make (3) actually work, tighten would need to adopt DCL's seg_start-tracking architecture (effectively a tighten-with-portal-continuation), which means reimplementing most of DCL's emission logic.  At that point, fold tighten into DCL — but DCL doesn't mutate spans, and tighten's mutation logic is complex (split, dominance, crossover).  The clean answer would be: a single combined function that does ONE walk doing BOTH emission (DCL-style with continuation) AND mutation (tighten-style with split logic).  That's a multi-day engineering effort I'm not going to complete autonomously here.  State preserved: (1)+(2) at 6.9% ccyc savings; tighten emission disabled. |
| 2026-05-01 |       —       |        — |     — |        — |     — |       — |     — | **Asymmetric top/bot anchors — INVESTIGATION (revised).** User suggestion: allow each span to carry independent (xlo, xhi) anchor pairs for its top vs bot edge, so when tighten narrows only one side, the unchanged side keeps old anchors+corners verbatim — no storage at (ox0, ox1) means no need to compute the OLD line's value at those points either, so we save **interps** (the dominant cost), not just stores. Added `TIGHTEN_STATS` instrumentation in `endpoint_spans.py` (per-tighten-call disposition counters: full_dom, no_cx top/bot/both/killed, crossover, anchor fast-path hits, plus a cheap pre-check `seg_top_max <= min(tl,tr)` that detects one-sided dominance from precomputed bbox values without per-x interp). `measure_tighten_disp.py` runs the 9-scene set and reports rates. Findings across 9 scenes: 89 tighten calls, 273 spans visited, 172 no-overlap, 12 full-dom, 101 narrowings — of which 35 are one-sided (28 bot-only + 7 top-only), 45 both-sides, 9 cross over. The cheap pre-check captures **34 of 35 one-sided narrowings (97%)** — `pre_top_dom` fires on 27 of 28 bot_only spans, `pre_bot_dom` fires on 7 of 7 top_only spans. **Pre-check alone (without asymmetric)** saves 2 NEW interps per one-sided fire (we still need OLD for storage at ox0/ox1) = 68 interps × ~80 cyc ≈ **5,500 cyc / 9 scenes**. **Pre-check + asymmetric** also skips OLD interps for the winning side (since we keep old line anchors verbatim) = 136 interps × ~80 cyc ≈ **10,900 cyc / 9 scenes** ≈ ~0.7% of total ccyc, ~1% on S1 baseline of 124k. Asymmetric anchors **double** the per-fire savings vs pre-check alone. **Cost**: span tuple grows 8→10 fields, POOL gains XLO_B/DEN_B (64 bytes), all interp paths in endpoint_spans.py (~30 sites referencing s[2..7]) and span_clip.asm (42 POOL_XLO/DEN sites) need per-side discrimination. Tiered approach makes sense: (a) add pre-check alone first (~50 ASM bytes, no structural change) for the 5.5k cyc; (b) consider asymmetric anchors later if the second half is worth the surgery. Pixel coverage preserved across 9 scenes (counters only, no behaviour change). |
| 2026-05-03 |       9083 B  |        — |     — |        — |     — |  +56% |     — | **Records-driven tighten — multi-record-aware ASM, no fallback.** User directive: "no fallback to ENTRY_TIGHTEN; records-driven must handle every case". Wrote `tfr_apply` as a multi-record iterator in 6502 ASM (span_clip.asm). Per span: bounded scan to find consecutive same-si records (CPY against precomputed end-of-buffer offsets to skip stale bytes past the record count — bug from earlier attempt), pre-check shortcut for the all-above/all-below case (emit unchanged via tg_append_x), iterate fragments with two cursors (top + bot record streams), advancing whichever has the smaller sox1, computing per-fragment cy via interp_store using the record's (sox0, cy0) → (sox1, cy1) line. tfr_apply_fragment factored out: handles one [ox0, ox1] fragment with given verdicts/cy values; allocates a new sub-span (narrow), emits an unchanged sub-span (full-dom), or skips (mark_solid). Wrapper still runs clip_line_records (yt + yb) followed by tighten_from_records — three 6502 entries per tighten. **Pixel test: all 9 scenes 0 lost / 0 gained — fully pixel-perfect.** **ccyc 1986k vs 1268k baseline = +56% slower** (clip_line_records does 4 aperture interps per span — top + bot, twice per side per call — vs ENTRY_TIGHTEN's 2). Cycle win requires the "free DCL records" path (use records written during draw_clipped_line emission, no extra clip_line_records walk) but that needs DCL CB-clip records to match clip_line_records semantics — first attempt at snapshot+restore in deferred replay produced 140-px divergence (DCL inside-fragment sox uses cb_cx aperture-clipped boundaries, clip_line_records uses sox at crossing-detected x-points; cy values differ per slot). Deferred for follow-up. ROM +1887 B (7196 → 9083). |
| 2026-05-02 |       7196 B  |        — |     — |        — |     — |  +56% |     — | **Records-driven tighten — Phase A WORKING, but slower (architectural validation only).** User suggested radical rewrite: have tighten consume DCL's per-span clip records instead of re-doing per-span interp. Implemented end-to-end in 6502: (1) `clip_line_records` ASM at $2018 — walks active span list, per span computes aperture endpoints + line endpoints + crossings, emits per-sub-range records (6 bytes each: si, sox0, sox1, verdict, cy0, cy1) to buffer at $0700 (top) / $0800 (bot). Verdict ∈ {above, inside, below}. Crossover detection uses existing compute_crossover routine; sub-range fragmentation matches Python prototype's clip_line_records. (2) `tighten_from_records` ASM at $201B — walks active span list, looks up records by si (linear scan, up to 1 record per side in v1), applies max/min narrowing, allocates fragments, copies POOL fields. Left/right fragment helpers preserve span portions outside seg range. (3) Python wrapper toggle `_USE_6502_RECORDS_TIGHTEN` in span_clip_6502.py: when set, _span_clip_6502.tighten() runs clip_line_records on yt/yb edges then tighten_from_records, instead of ENTRY_TIGHTEN. (4) Multi-record fallback: ASM tighten_from_records v1 only handles single-record-per-side cases. For crossover spans (4 of 106 tighten calls in 9 scenes have multi-records), fall back to regular ENTRY_TIGHTEN. Detection in Python (~negligible cost). **Pixel test: all 9 scenes 0 lost / 0 gained — fully pixel-perfect.** **ccyc: 1888k vs 1210k regular = +56% slower.** Phase A characteristic — clip_line_records duplicates DCL's per-span clip work. Phase B (modify DCL to write records during its existing walk, eliminating duplicate) deferred — would save ~3% projected. Architectural validation complete; records-driven 6502 design is correct and pixel-equivalent. ROM +763 B (6433 → 7196). Several critical bugs fixed during implementation: (a) zp_clr_save_x not set on early-skip path → walk used wrong span; (b) umul8 clobbers zp_tmp0 → cy_lo storage corrupted; (c) line xl/xr swap broke when sx2 wraps past 255; (d) tighten_from_records narrow path needed left/right fragment handling for partial coverage. New entry points in jump table at $2018, $201B. Records buffers at $0700, $0800 (256 B each). Toggleable via `endpoint_spans._TIGHTEN_MODE='records'` (Python) and `span_clip_6502._USE_6502_RECORDS_TIGHTEN=True` (ASM). |
| 2026-05-01 |       6433 B  | **122921**         | −809  | **252242**         | −1168 | **375163**| −1977 | **One-sided dominance pre-check (tier 1.5).** Extends the existing post-old-interp dominance check at `old_done` from full-dom-only to a three-way fork: full dom (existing shortcut), top-only dom, bot-only dom, neither. When only top dominates (ot_l/r >= bb_yt_max but bot doesn't), set zp_nt_l/r=0 (sentinels for "no narrowing on top": max(ot,0)=ot, no top crossover, c_tl=clamp(0)=Y_BIAS<=ot_l), set zp_pre_dom_flags=$01, fall through to gated new interp. Symmetric for bot-only with $FF sentinels. New `new_slow_gated` path: top half + bot half each gated on a flag bit, computes only the non-dominated side. `tg_pod_skip` trampolines to `new_slow_gated` when flags non-zero (bypassing cache + anchor + constant-line fast paths that would overwrite sentinels). Cache update at `new_done` invalidates (zp_cache_ox1=$FF) AND sets zp_cache_nt/nb=$FF off-screen sentinels so the running seg bound narrowing skips this span (its `CMP #(VIS_YMAX+1)` rejects $FF). Flag cleared at tg_go (entry) and tg_newdom_fast (which bypasses old_done). New ZP slot `zp_pre_dom_flags=$B6` reuses CB clip slot (non-overlapping with tighten). 10-scene grand total **1214404 → 1205692 = −8712 cyc (−0.72%)**. Per-scene: spawn-W −2.0%, SE −1.4%, S2_NE −1.3%; +0.6% on N and +0.3% on T_N where per-span overhead exceeds savings. ROM +159 B. Pixel-perfect preserved across 9 reference scenes. |
