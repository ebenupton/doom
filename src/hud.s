...
;
; Renders "X=hhhh Y=hhhh R=hh" (map-relative prescaled position integer,
; s16 hex, and the view angle byte) into the BACK buffer's first character
; row, after the frame render and before the flip, using the OS ROM font.
;
...
;   $3D83/84 = x int lo/hi, $3D86/87 = y int lo/hi, $3D89 hud_en,
;   $3D8A hud_prev (toggle edge state).  The driver's hud_glue pages
;   BANK_C and JSRs hud_draw ($A400) when hud_en is nonzero.
;
