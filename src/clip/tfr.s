...
; test; this chain reloads for every pair, so reordering is free.)
   LDA POOL_BDEN,Y
   CMP POOL_BDEN,X
   BNE ta_link                          ; 85.9% reject
   LDA POOL_TDEN,Y
   CMP POOL_TDEN,X
   BNE ta_link                          ; 11.5% reject
   LDA POOL_XEND,Y
   CMP POOL_XSTART,X
   BNE ta_link                          ; 3.8% reject
   LDA POOL_BXLO,Y
   CMP POOL_BXLO,X
   BNE ta_link                          ; 82.1% reject
   LDA POOL_TL,Y
   CMP POOL_TL,X
   BNE ta_link                          ; 9.0% reject
   LDA POOL_TR,Y
   CMP POOL_TR,X
   BNE ta_link                          ; 10.3% reject
   LDA POOL_TXLO,Y
   CMP POOL_TXLO,X
   BNE ta_link                          ; 11.5% reject
   LDA POOL_BL,Y
   CMP POOL_BL,X
   BNE ta_link                          ; 52.6% reject
   LDA POOL_BR,Y
   CMP POOL_BR,X
   BNE ta_link                          ; 46.2% reject
...
;  the symbol, this store, and no reader at all in any .s/.inc/.py/.asm.
;  DCL's portal continuation went with the portal check on 2026-08-22;
;  this was its last surviving limb.  $BA is free.)
