...
ANIM_SSMASK = $1100                     ; MAIN, BOTH builds (unforked
                                        ; 2026-07-21): u8 mover bitmask per
                                        ; subsector, read by the hub with
                                        ; zero paging; anim_init copies it
                                        ; down from the build's staged SRC
                                        ; (a WHOLE page — do not park anything
                                        ; in the $1100 tail). $1F00 -> $1100
                                        ; 2026-08-17: the page the retired
                                        ; cadence probe freed, taken so the
                                        ; driver could slide down off $2000
                                        ; and CODE could claim $2B00.
.if ::BANKED
ANIM_TABL0  = $BE90                     ; L0: 6 u16 ptrs -> per-mover blocks
ANIM_CFG    = $B300                     ; L2: 12 B per mover (bounds/speed/waits)
ANIM_SSMASK_SRC = $B400                 ; L2: staging page — the disc can only
                                        ; ship bank contents (LOW loads $1B40)
.else
ANIM_TABL0  = $E600
ANIM_CFG    = $E700
ANIM_SSMASK_SRC = $E500                 ; flat TABLES block page (shipped in
                                        ; the copro DATA file; was the flat
                                        ; RUNTIME home before the unfork)
.endif
...
; file starts at $8600), so the mask ships at the build's staged SRC
; and is copied down here. History: the 355c126 reshuffle moved banked
; SSMASK to $0A80 WITHOUT this copy — the model harness pokes main RAM
; directly, so every banked gate passed while the DISC ran against OS
; boot junk and doors froze (found 2026-07-17; jsbeeb is the catcher).
   LDX #0
ai_cp:
