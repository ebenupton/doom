...
; DEFQ storage (ONE-SEG-LAG regime 2026-07-16: the queue never holds
; more than TWO ops — the pending previous sibling and the fresh one —
; but keeps the proven variable-length append/apply machinery).
DEFQ_BASE = $0600                       ; 256 bytes (free: span pool ends $059F)
DEFQ_TAIL = zp_defq_tail                ; queue tail offset (u8)
DEFQ_HEAD = zp_defq_head                ; first un-applied op's offset
DEFQ_OVF  = $09FC                       ; overflow debug flag
...
;         if is_full(): break
; defq_apply_one — apply the op at DEFQ_HEAD, advance the head.
; ONE-SEG-LAG core: the seg loop calls this once per seg when an older
; sibling's op is pending (queue depth 2); defq_drain finishes the last
; op at subsector end. (The old is_full early-out is gone — the python
; mirrors apply unconditionally, and post-full ops are no-ops.)
defq_apply_one:
.scope
   LDX DEFQ_HEAD
...
; defq_lag_drain — apply older ops while MORE than one is queued
; (mirrors python's `while len(deferred) > 1`; depth is <= 2 by
; construction, so this is at most one application per seg).
defq_lag_drain:
.scope
dl_more:
   LDA zp_defq_nops
...
; defq_drain — apply everything still pending (subsector end).
defq_drain:
.scope
dd_more:
   LDA DEFQ_HEAD
