...
;                 per-frame vertex-cache mode chosen (vrcache_frame).
...
; (no-op macro on flat; callers re-page before their next engine call).
   ...
   PAGE BANK_WALK
                                        ...
                                        ; for vrcache_frame below.)
   vrcache_frame                           ; translation-coherence vertex cache
   RTS
.endscope
...
;  vfoff vector target and in the vrcache_on cold arms; single callers all.)


; (br_to_view_fetch fully retired 2026-07-27 round 2: the vrcache cold
...
;  position path rides rot_w_pages too — vrcache_frame decomposes
...
;   Callers: seg_xform vrcache_on cold + vfoff (both sides), lo.s cr_plain.
