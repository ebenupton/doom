...
; transform with RAM vxcache" + reciprocal + X-projection phase of Python's
...
; header.s) plus a 1-bit-per-vertex valid bitmap at VXCACHE_VALID_BASE
...
; (VRCACHE_WARM_ARM folded into the vrcache_on island 2026-08-09 — single use)
...
; (vfoff plain fetch, near-clip, recip, fills, the vrcache_on island). The
; halves only connect through JMP (vec) -> the ::-global vfoff/vrcache_on
...
; into vrcache_on's VRCACHE_VALID accesses.
...
; paged (the load-bearing exit postcondition, unchanged).
   ...
   AND VXCACHE_VALID_BASE,Y
   BEQ vmiss
; --- vxcache hit serve (was VC_HIT_ARM, absorbed 2026-08-09) ---
...
; (vrcache_on island lives at the BODY END — vector-entered and JMP-exited,
...
; verdicts store plane+struct symmetrically, 2026-07-27)
   LDA VXCACHE_VALID_BASE,Y
   ORA zp_seg_v_bitm
   STA VXCACHE_VALID_BASE,Y
; --- VECTORED fetch dispatch (Eben, 2026-07-27: the bca-cache idiom —
; pointer, not flag; vrcache_frame aims it once per frame). Cache off:
; straight into the inline plain fetch below. Cache on: the vrcache_on
...
.macro SXV_BOT pg, vfoff, vrcache_on, rwpa, rwpb
.scope
::vfoff:
...
::vrcache_on:
; --- VRCACHE serve, TRUE16 (2026-08-10): the cache memoizes base counts =
...
; ref_c, so warm == birth == Python bit-exactly BY CONSTRUCTION.
   LDA VRCACHE_VALID,Y                         ; Y = B RIDES from the head
   ...
   LDX zp_seg_v_idx_l
   LDA VRCACHE_XLO+pg,X                        ; warm: base counts -> the
   STA zp_br_vx_l                          ; working slots
   LDA VRCACHE_XHI+pg,X
   STA zp_br_vx_h
   LDA VRCACHE_YLO+pg,X
   STA zp_br_vy_l
   LDA VRCACHE_YHI+pg,X
   STA zp_br_vy_h
   JMP vxq_add
vs_cold:
   LDA VRCACHE_VALID,Y
...
::rwpb:
   ...
   LDY zp_seg_v_idx_l
   LDA zp_br_vx_l
   STA VRCACHE_XLO+pg,Y                        ; birth store: counts verbatim
   LDA zp_br_vx_h
   STA VRCACHE_XHI+pg,Y
   LDA zp_br_vy_l
   STA VRCACHE_YLO+pg,Y
   LDA zp_br_vy_h
   STA VRCACHE_YHI+pg,Y
vxq_add:
...
; senior test anywhere (probe, fetch, VRCACHE, fills all baked).
::sx_vert_lo:
   SXV_TOP 0, zp_vf_vec0
::sx_vert_hi:
   SXV_TOP $100, zp_vf_vec1
   SXV_BOT 0, sxv0_vfoff, sxv0_vrcache_on, sxv0_rwpa, sxv0_rwpb
   SXV_BOT $100, sxv1_vfoff, sxv1_vrcache_on, sxv1_rwpa, sxv1_rwpb

; (vrcache_store_tail deleted 2026-08-09 — birth store inlined per side in
;  the vrcache_on islands, side baked)



; (VRCACHE_WARM_ARM moved above SXV_BODY 2026-08-09 — it expands inside it now)

; ============================================================================
; (VRCACHE serve INLINED into SXV_BODY's vrcache_on islands 2026-08-09 — the
; VRCACHE_SERVE_SIDE macro and the standalone vrcache_serve_lo/hi bodies are
; gone. In/out contract unchanged: in zp_seg_v_idx_l/b + zp_seg_v_bitm
; + vrcache_ref_x/y; out zp_br_vx/vy lo/hi/ext, bit-identical to
; br_to_view — base' = L(w) is translation-invariant, see vrcache.s.)
; ============================================================================
