...
zp_plot_i = $95
   ...
   .include "../abi.inc"   ; HUD_FONT_B / HUD_FONT_MASTER
...
; ORG $1900 -> the linker config.  The three exports are for
; tools/test_hud_font.py, which reads them out of ld65's label dump.
.export start, hudprobe, hudbase
.export main, huden, bufhi, skipchk   ; tube/test_hostt_hud.py
...
hgnopg:
    ...
    JSR hudhex
    LDA #' '
    JSR hudchar   ; F = PAL fields the last frame consumed,
    LDA #'F'
    JSR hudchar   ; i.e. how many 1/50ths that frame cost --
    LDA #'='
    JSR hudchar   ; the copro's own frame-rate readout
    LDA hudb+10
    JSR hudhex
.if ::T_TRIPWIRE
    LDA #' '
    JSR hudchar   ; TRIPWIRE latch: the id of the EARLIEST
    LDA #'T'
    JSR hudchar   ; checkpoint that saw the watched byte
    LDA #'='
    JSR hudchar   ; corrupted. 00 = nothing has tripped.
    LDA hudb+9
    JSR hudhex
.endif
...
hudrom:
    .byte 0   ; ROMSEL value the OS was using at boot
hudbase:
    .word 0   ; found font base; &FFxx = searched, none
hudt:
    .word 0   ; hudchar scratch
huden:
    .byte 0
hudprev:
    .byte 0
xsent:
    .byte 0   ; last button byte shipped; starts 0 (an
...
hostend:
   .assert hostend <= $4400, error   ; carousel buffer B0 starts at &4400
; SAVE "HOSTT", start, hostend -> the build script
