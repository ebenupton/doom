...
; --- endpoint sort: ensure y0 >= y1; dy = |y0-y1| ---
    ...
    BCS dyok
    SBC #0
    EOR #$FF   ; y0 < y1: negate (SBC #0 exits with C=1)...
    LDX x0
    LDY x1
    STY x0
    STX x1   ; ...and swap the endpoints
    LDY y1
    STY y0
    dyok: STA dy
...
;  the old staged zero + double ADC was the 320-wide relic)
    ...
    LDA x0
    AND #7
    TAY   ; Y = start bit -> strtN dispatch index
    CPX dy
    BCC steep_hop   ; steep rare (0 on suite): hop below
...
;     within the char row, i.e. the (scr),Y offset). C=1 on entry. ---
    backh: PLP
    BCC right   ; banked x-compare carry: C=1 -> leftward
    LDA strt1,Y
    STA x1
    LDA strt1+8,Y
    STA y1
...
.if ::STEEP_COMPACT
    LDA stmask,Y
    STA b   ; single-pixel mask for x0&7 (Y = x0&7 here)
    PLP
    BCS left   ; banked x-compare carry: C=1 -> leftward
    STX sr_dx+1   ; SMC: core's SBC #dx (X = dx, or 1 if vertical)
    LDA dy
    STA sr_dy+1   ; SMC: core's ADC #dy
    LDA y0
    AND #7
    TAY   ; Y = row within char row
    LDX errs
    SEC
    JMP sr_loop   ; X = r = dy>>1 (or dy); C=1 loop invariant
.else
    PLP
    BCS left
    LDA strt2,Y
...
.if ::STEEP_COMPACT
    ...
    LDA dy
    STA sl_dy+1   ; SMC: core's ADC #dy
    LDA y0
    AND #7
    TAY
    LDX errs
    JMP sl_loop   ; C=1: the BCS that chose the left arm
.else
    LDA strt3,Y
    STA x1
    LDA strt3+8,Y
...
; --- right-going blocks: masks &80,&40..&01; scr steps right ---
    a00: LDA err
    LDX #$FF   ; run starts at bit 0: seed mask = all 8 bits
    b00: SBC #dy
    BCS b10
    ADC dx
    STA err   ; (dy/dx immediates SMC-patched)
    LDA #$80
    ORA (scr),Y
    STA (scr),Y   ; plot run [bit 0..bit 0]
    DEC cnt
    BEQ e00
    f00: DEY
    BPL a10   ; row step; new run at bit 1
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a10   ; (256-wide: the LO() subtract was
...
;  blocks 7 keep the LIVE -248 form)
    ...
    BEQ d00
    LDA err
    SBC errs   ; ls tail: final half-step test
    STA err
    INC cnt
    BCS f00
    d00: RTS
...
; (&FF>>M) AND ~(&FF>>(N+1))
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a20   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a30   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a40   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a50   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a60   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a70   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
...
; byte to the right
    ...
    BEQ e70
    f70: DEY
    BPL ad0   ; y-step: next byte (same rows)
    LDA scr
    SBC #<(SCREEN_WIDTH-8)
    LDY #7
    STA scr   ; combined char-row
    LDA scr+1
    SBC #>(SCREEN_WIDTH-8)
    STA scr+1
    JMP a00   ; wrap + byte right
    e70: DEC ls
    BEQ d70
    LDA err
    SBC errs
...
;     the above; block digit still = distance along drawing direction) ---
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a11   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a21   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a31   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a41   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a51   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a61   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
    ...
    DEC scr+1   ; char-row descend: scr -= 256
    LDY #7
    BCS a71   ; (256-wide: the LO() subtract was
;  an 8-cycle no-op under the C=1
;  invariant and the BCS is always
;  taken — 320-wide relic, 2026-07-27;
;  blocks 7 keep the LIVE -248 form)
...
; cross into the next byte to the LEFT
    ...
    BEQ e71
    f71: DEY
    BPL sb1   ; y-step: next byte (same rows)
    LDA scr
    SBC #<(SCREEN_WIDTH+8)
    LDY #7
    STA scr   ; combined char-row
...
; protocol. One pixel per iteration, rows bottom-to-top.
    ...
    TAX
    DEY
    BPL sr_loop   ; same column, row above
    LDY #7
    DEC scr+1   ; char-row wrap: up one page
    BCS sr_loop   ; (C=1 preserved: SBC didn't borrow)
    sr_x:
    sr_dy: ADC #0   ; r += dy (SMC); always carries out (dy > dx)
    DEC cnt
    BEQ sr_e   ; all dx column steps done -> ls tail
    sr_n:
    TAX
    ...
    BMI sr_row
    SEC
    BCS sr_loop   ; (SEC restores the C=1 loop invariant)
    sr_row:
    LDY #7
    DEC scr+1   ; char-row wrap after the column step
    SEC
    BCS sr_loop
    sr_wrap:
    ...
    LDA scr
    ADC #8
    STA scr   ; scr one byte right (C=0 -> exact +8)
    BCS sr_hi
    DEY
    BMI sr_row
    SEC
...
; shifts left with ASL/ROL, byte cross steps scr -8)
    ...
    LDA scr
    SBC #7
    STA scr   ; scr one byte left (C=0 -> exact -8)
    BCC sl_hi
    DEY
    BMI sl_row
    SEC
...
; stmask[x&7] = single-pixel mask &80>>(x&7)
    stmask: .byte $80,$40,$20,$10,$08,$04,$02,$01
.else
; ------------------------------------------------------------------
