...
BCA_AB = 0x0062  # view angle byte — ZP (zp_buf's slot, freed with span_read 2026-07-26; NOT $64 — that is zp_bv_entry's HI byte, the drivers seed it); poked per frame by driver/harness; vrcache_ab aliases it; zp.inc aliases bca_ab = BCA_AB
SQR_MIR_LO = 0x0200  # quarter-square EVEN-MIRROR lo page: [k] = f(256-k) for k 1..255, [0] = f(256)&$FF. Lets a diff-side walk LDA (SQR_LO-M),X reach f(|o-M|) for ANY M 0..256 with a 2-byte SMC base and no abs staging (the t16p2 trick, 2026-09-01). Boot-generated by the sqr_fill mirror tail.
SQR_LO = 0x0300  # qsqr lo bytes (f 0..255). The quad grew to [mir][lo,2lo][mir][hi,2hi] at $0200-$07FF when the low strip moved up: each mirror sits DIRECTLY below its plane, which is the whole point.
SQR2_LO = 0x0400  # qsqr lo bytes (f 256..510) — must stay adjacent above SQR_LO (the base+mag walks index across the seam).
...
VRCACHE_STATE_LEN = 0x0100  # bytes to zero at boot (the whole bitmap page)
...
WALKTAB_BASE = 0xB2E4  # USETAB + 1 + n_use*11: the walk-over record section (n_walk byte, then 11-byte records — 9 + 2 biased hi-byte y bounds, SAME stride as use records). colmap asserts n_use == 9
WALKTAB_BASE_FLAT = 0x8AE4
USETAB_BASE = 0xB280  # use + walkover line tables (u8 n, n x 9: x1,y1,dx,dy s16 + action); banked home is BANK A since the slide arc — pmove_use pages SEG for the list reads
USETAB_BASE_FLAT = 0x8A80
