...
MAIN_BASE = 0x2B00  # engine CODE region head (cfg-anchored; MAIN first). $2C00 -> $2B00 2026-08-17: SSMASK to the freed $1100 page let the driver+PMOVE block slide a page down, so CODE claims $2B00-$2BFF (+256 B of tail slack). Flat matched by moving the CPM psi planes off $2B00.
...
DRV_ORG = 0x1F00  # walk/anim driver entry (!BOOT CALLs this). $2000 -> $1F00 2026-08-17: ANIM_SSMASK vacated $1F00 for the freed $1100 page, so the whole driver + PMOVE block slid one page down and CODE gained $2B00-$2BFF.
DRV_VARS = 0x2080  # walk driver variable block (layout below)
DV_ANGIDX = 0x2080  # view angle index 0..63 (angle byte = idx*4)
DV_BACKHI = 0x2081  # hidden-buffer page hi ($58/$6C)
DV_PXF = 0x2082  # player x 8.8 prescaled, 24-bit: frac
DV_PXL = 0x2083  # ... int lo
DV_PXH = 0x2084  # ... int hi
DV_PYF = 0x2085  # player y frac
DV_PYL = 0x2086  # ... int lo
DV_PYH = 0x2087  # ... int hi
DV_JIDX = 0x2088  # vsync journal index
DV_HUD_EN = 0x2089  # debug HUD on/off (H toggles)
DV_HUD_PREV = 0x208A  # H-key debounce state
DRV_GLUE = 0x20A0  # anim/HUD glue pocket
DRV_CLR = 0x2100  # input block + flip scheduler; the unrolled framebuffer clears moved to BANK C 2026-08-16, and the whole driver slid $2200 -> $2100 with DRV_ORG 2026-08-17 (2026-08-14: the sincos overlay moved to bank A $BA00 with STEPTAB/USEVEC; the driver packs below the engine PMOVE region)
...
PM_MOMX = 0x03F8  # player momentum x (s16 8.8 prescaled) — the COLPORT page tail; anim_init's 512B copy-down zero-seeds it (bank staging pads with zeros)
PM_MOMY = 0x03FA  # ... momentum y
PM_TICREM = 0x03FC  # 35Hz tic accumulator remainder (0..9)
