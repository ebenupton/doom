...
def _joint_pass(si, clips, ctx, vz, surface, vxcache, vwh_cache):
    ...
    for vidx in (s[0], s[1]):
        ...
        if vxcache[vidx] is None:
            vxcache[vidx] = dw.fp_to_view(fpv[vidx][0], fpv[vidx][1], ctx)
        evx_t, evx_r, evy, fvx, vy_idx = vxcache[vidx][:5]
        if evy < 1:
            continue
...
def _seg_hook(si, clips, ctx, vz, surface, vxcache, vwh_cache, deferred=None):
    if MODE_B[0]:
        _joint_pass(si, clips, ctx, vz, surface, vxcache, vwh_cache)
    _orig_seg(si, clips, ctx, vz, surface, vxcache, vwh_cache, deferred)
