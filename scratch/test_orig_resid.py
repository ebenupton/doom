...
def disp(B,t):
    ...
    for x in range(W):
        ...
        if len(bo): bad+=int((np.full(len(bo),99) if len(ay)==0 else np.abs(ay[None,:]-bo[:,None]).min(axis=1)>2).sum())
    ...
    return bad
print("ORIGINAL buggy angle method vs corner-proj:")
for (px,py,ab) in [(1056,-3616,137),(1354,-3748,95),(893,-3218,123),(973,-3367,239)]:
    t=render(px,py,ab,False); a=render(px,py,ab,True)
    print(f"  ({px},{py},{ab}): orig-angle disp={disp(a,t)}")
