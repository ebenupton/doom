#!/usr/bin/env python3
"""Compare ClipSpans vs ClipColumns output by capturing all draw.line calls."""
...
# ── ClipColumns (reference, known good) ──
class ClipColumns:
    ...
    def draw_clipped(self, lines, color, surface):
        ...
        for lx1, ly1, lx2, ly2 in lines:
            ...
            if abs(dx) < 0.5:
                ...
                ybb = min(ybb, yb)
                if ya < ybb:
                    pygame.draw.line(surface, color, (ix, int(ya)), (ix, int(ybb)), 1)
            else:
                inv_dx = 1.0 / dx
                seg_start = None
...
# ── Import ClipSpans from the main module ──
# We'll import it dynamically to get the latest version
import importlib.util
spec = importlib.util.spec_from_file_location("doom_mod", "/Users/ebenupton/doom/doom_wireframe.py")
# Can't import directly since module has a main loop -- extract ClipSpans manually
# Instead, exec just the relevant parts

_clip_spans_src = open("/Users/ebenupton/doom/doom_wireframe.py").read()
# Find the class and helpers we need
import re

# Execute just the parts we need in a namespace
ns = {"math": math, "pygame": pygame, "WIDTH": WIDTH, "HEIGHT": HEIGHT}
# Extract from the comment block through the class
lines_src = _clip_spans_src.split('\n')
start = None
end = None
for i, line in enumerate(lines_src):
    if '# -- Trapezoid span clipper' in line or '# ── Trapezoid span clipper' in line:
        start = i
    if start is not None and line.startswith('# ── View-space') or (start is not None and line.startswith('# -- View-space')):
        end = i
        break
if start is not None and end is not None:
    clip_code = '\n'.join(lines_src[start:end])
    exec(clip_code, ns)
    ClipSpans = ns['ClipSpans']
else:
    print("ERROR: Could not extract ClipSpans from doom_wireframe.py")
    sys.exit(1)


# ── BSP render function (parameterized by clip class) ──
...
class CaptureSurface:
    """Wrapper that captures draw.line calls."""
    pass

_real_draw_line = pygame.draw.line

def capturing_draw_line(surface, color, p1, p2, width=1):
    if isinstance(surface, CaptureSurface):
        surface.calls.append((p1[0], p1[1], p2[0], p2[1]))
    else:
        _real_draw_line(surface, color, p1, p2, width)

pygame.draw.line = capturing_draw_line

def render_scene(clip_class, px, py, ang):
    cos_a, sin_a = math.cos(ang), math.sin(ang)
    vz = player_floor(px, py) + 41.0
    surf = CaptureSurface()
    surf.calls = []
    sys.setrecursionlimit(10000)
    clips = clip_class()
    render_bsp(len(nodes) - 1, clips, cos_a, sin_a, px, py, vz, surf)
    return surf.calls


def compare_at(px, py, ang, label=""):
    col_calls = render_scene(ClipColumns, px, py, ang)
    span_calls = render_scene(ClipSpans, px, py, ang)

    col_set = set(col_calls)
    span_set = set(span_calls)

    only_col = col_set - span_set
    only_span = span_set - col_set

    print(f"\n{'='*60}")
    print(f"  {label}")
    print(f"  pos=({px:.0f}, {py:.0f}) angle={math.degrees(ang):.1f}")
    print(f"{'='*60}")
    print(f"  ClipColumns lines: {len(col_calls)}  (unique: {len(col_set)})")
    print(f"  ClipSpans   lines: {len(span_calls)}  (unique: {len(span_set)})")
    print(f"  Missing from spans (lost geometry):    {len(only_col)}")
    print(f"  Extra in spans (punch-through):        {len(only_span)}")

    if only_col:
        print(f"\n  First 20 lines MISSING from spans (in columns only):")
        for i, (x1, y1, x2, y2) in enumerate(sorted(only_col)[:20]):
            print(f"    ({x1},{y1}) -> ({x2},{y2})")

    if only_span:
        print(f"\n  First 20 EXTRA lines in spans (punch-through):")
        for i, (x1, y1, x2, y2) in enumerate(sorted(only_span)[:20]):
            print(f"    ({x1},{y1}) -> ({x2},{y2})")

    return len(only_col), len(only_span)


# ── Test from several angles ──
if __name__ == "__main__":
    test_cases = [
        (player_x, player_y, math.radians(pangle), "Spawn position"),
        (player_x, player_y, math.radians(pangle) + 0.3, "Spawn +17deg"),
        (player_x, player_y, math.radians(pangle) - 0.3, "Spawn -17deg"),
        (player_x, player_y, math.radians(pangle) + math.pi/2, "Spawn +90deg"),
        (player_x + 200, player_y, math.radians(pangle), "Spawn +200x"),
        (player_x, player_y + 200, math.radians(pangle), "Spawn +200y"),
    ]

    total_missing = 0
    total_extra = 0
    for px, py, ang, label in test_cases:
        m, e = compare_at(px, py, ang, label)
        total_missing += m
        total_extra += e
