...
def mk_bare():
    ...
    # (ANIM_SSMASK mirror-copy deleted 2026-08-19: the mask's home is the
    # bank-B page the L2 image already carries — nothing to copy.)
    # sqr quad: GENERATED by anim_init on hardware (it lives on the OS
    # pages, unshippable) — we enter engine entries directly and never run
    # init, so mirror the generator's output here, like the SSMASK copy.
    import abi as _abi
    for i in range(0x400):
        bare[_abi.SQR_BASE + i] = src.bm[_abi.SQR_BASE + i]
    for a,v in ZP.items(): bare[a]=v
    # (the RNS stack-page copy died here 2026-08-18: the vectoring block
    #  left page 1 on 2026-07-12 — this was copying bank-B bbox bytes onto
    #  the stack page for nothing)
...
def run_entry(sc, entry, maxc=10_000_000):
    mpu=sc.mpu; mpu.pc=entry; mpu.sp=0xDD; mpu.p=0x34  # SP capped below SQR_MIRROR ($01E0-$01FF, the stack-page mirror)
    mem=mpu.memory; mem[0x01DF]=0xFE; mem[0x01DE]=0xFF
    for _ in range(maxc):
