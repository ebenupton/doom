...
import abi
import symmap
ENTRY_VIEW = symmap.sym('view_setup', banked=1)
ENTRY_RENDER = symmap.sym('render_frame', banked=1)
...
def mk_bare():
    ...
    LOW = bytes(src.bm[abi.LOW_BASE:0x5800])   # THE FRAMEBUFFER WALL: LOW
        # ships $0F00-$57FF and nothing above it (build_anim_ssd cuts the
        # disc image at the same wall).  The old MAIN_BASE + getsize(
        # 'bsp_render_bk.bin') proxy named a file this build no longer
        # writes -- the driver and the engine are ONE region now.
    ...
    # lockstep RENDER
    m=msc.mpu; b=bsc.mpu
    m.pc=ENTRY_RENDER; m.sp=0xDD; m.p=0x34; m.memory[0x01DF]=0xFE; m.memory[0x01DE]=0xFF
    b.pc=ENTRY_RENDER; b.sp=0xDD; b.p=0x34; b.memory[0x01DF]=0xFE; b.memory[0x01DE]=0xFF
