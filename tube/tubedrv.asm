...
.bootend
ASSERT bootend <= &7C00         \ boot must fit the X-plane hole; DATA
                                \ starts at &7C00 (VDESC/sincos)
SAVE "COPROT", &7800, bootend
...
.spraw
    EQUB 0                      \ button LEVELS, from the last b7=1 byte
...
.o_prev
    EQUB 0                      \ O press-edge state (masked b1 value)
...
.hudmk
    ...
    DEX
    BNE hudmk
    LDA T_DV_ANGIDX : JSR esend1
    LDA T_DV_PXF    : JSR esend1
    LDA T_DV_PXL    : JSR esend1
    LDA #0          : JSR esend1
    LDA T_DV_PXH    : JSR esend1
    LDA T_DV_PYF    : JSR esend1
    LDA T_DV_PYL    : JSR esend1
    LDA #0          : JSR esend1
    LDA T_DV_PYH    : JSR esend1
IF T_TRIPWIRE
...
.sptab                          \ spawn pose, DV_* +0..+7 order
    EQUB SPAWN_ANGIDX, 0, SPAWN_PXF, SPAWN_PXL, SPAWN_PXH
    EQUB SPAWN_PYF, SPAWN_PYL, SPAWN_PYH
...
\ two names.)
.resend
ASSERT resend <= &F800          \ RESIDENT must fit under the client OS
SAVE "COPRES", RESIDENT, resend
