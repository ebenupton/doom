...
def write_tube_syms():
    ...
    # The HUD packet's last byte is zp_bv_entry's RAW low byte (the class
    # letter); a class entry ending in FE/FF would let the packet fake the
    # host's FE FE FE FE / FF FF FF FF markers.
    for _n in ('dbox_check', 'bbox_check_angle', 'box_classify'):
        assert fsym(_n) & 0xFF < 0xFE, f'{_n} low byte {fsym(_n) & 0xFF:02X} would fake a wire marker'
    with open('tube/tube_syms.inc', 'w') as f:
        ...
        for t, s in (('T_VIEW_SETUP', 'view_setup'),
                     ...
                     # is P/R/D decoded from zp_bv_entry's low byte against
                     # these two class entries, as src/hud.s does
                     ('T_DBOX_CHECK', 'dbox_check'),
                     ('T_BBOX_CHECK_ANGLE', 'bbox_check_angle'),
                     ('T_CPM_KDXH', 'CPM_KDXH'),
