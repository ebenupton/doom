...
def asm(src, out):
    """Assemble a boot stub with ca65/ld65 (beebasm was retired 2026-09-05;
    every output was proved byte-identical first).  `src` names the ca65
    source stem under src/boot, `out` the config's output suffix."""
    sys.path.insert(0, os.path.join(ROOT, 'tools'))
    import build_boot
    return build_boot.read(src, ('BANKED=1',) if src == 'hostg' else ())
...
def main():
    ...
    with open('tube/tube_loads.inc', 'w') as f:
        f.write("; GENERATED by tube/build_tube_game.py - DO NOT EDIT.\n")
        f.write('    .byte "LOAD CODE"\n    .byte 13\n')
        f.write('    .byte "LOAD DATA"\n    .byte 13\n')
        f.write("    .byte 0\n")

    # ---- assemble the programs ----
    detect = asm('detect', 'DETECT')
    hostt = asm('hostg', 'HOSTT')
    ...
    sys.path.insert(0, os.path.join(ROOT, 'tools'))
    import build_boot
    _td = build_boot.build('tubedrv', ('BANKED=0',))
    coprot = open(_td + '.boot', 'rb').read()
    copres = (open(_td + '.res', 'rb').read() +      # the SKIPTO pad is in the
              open(_td + '.emit', 'rb').read())      # config's fill now
