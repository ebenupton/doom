        ...
        for py in range(-3800,-3300,50):
            for ab in range(0,256,8):
                ...
                for nid in range(len(dw.nodes)):
                    node=dw.nodes[nid]
                    for side in (0,1):
                        truth=dw.fp_bbox_visible_fixed(node,side,ctx)  # corner-proj
                        ang=ang_br(node,side,ctx,ab)
                        total+=1
                        if truth!=ang:
                            bad+=1
                            if len(samples)<6: samples.append((px,py,ab,nid,side,truth,ang))
    print(f"{label}: {bad} mismatches / {total} checks")
    for s in samples: print("   ",s)
A._STRADDLE_FULL=False; sweep("UNGUARDED (current)")
A._STRADDLE_FULL=True;  sweep("GUARDED   (proposed)")
